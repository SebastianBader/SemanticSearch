...
# on streamlit - there is an old version of sqlite3!!!
# the following lines replace it with an up-to-date version
import pysqlite3
import sys
...
st.header(f"_:blue[Semantic Search based on Vector Embeddings]_")
st.markdown(
    f"The following search query, will be embedded into a {collection._model.dimension}-dimensional vector."
    f" Afterwards, this embedding is used to identify the {n_results} closest documents withn the collection _'{collection_name}'_ according to the '{collection.get_model().metadata.get('hnsw:space')}' similarity."
